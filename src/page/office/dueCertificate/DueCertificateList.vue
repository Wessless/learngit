<template>
    <div class="expense">
        <chat-header :showBack="true" :title="'应付款凭证'" :showRightBtn="false"></chat-header>
        ...
        <div class="dueList">
            <due-certificate-item v-for="item in dueList" :key="item.num" :item="item" @clickChangeChargeDate="clickChangeChargeDate"></due-certificate-item>
        </div>
...
import { changePaymentChargecheckDate,findPaymentChargecheckList } from '@/js/api'
import { showLoading,closeLoading,alertError,dateTimeFormatter} from '@/config/utils'
import { mapState, mapMutations } from 'vuex'
import ChatHeader from '@/components/chat/ChatHeader'
import dueCertificateItem from '@/page/office/dueCertificate/DueCertificateItem'
...
export default {
    ...
    methods:{
        loadList(){
            ...
            findPaymentChargecheckList(staffID,this.formInline.daterange[0],this.formInline.daterange[1],this.formInline.sort).then((result)=>{
                closeLoading(loading);
                this.dueList = result.data.data;
        ...
        confirmPayment(){
            ...
            if(chargeDate==""&&this.form.isToday==false){
                ...
                changePaymentChargecheckDate(this.currChargeBillID,this.form.isToday?'1':'0',chargeDate).then((result)=>{
