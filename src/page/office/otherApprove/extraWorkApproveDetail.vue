<template>
    <div class="businessApproveDetail">
        ...
        <div class="detailList">
            ...
            <div class="detailItem">
                <div class="detailTitle">代申请人</div>
                <div class="detailContent">{{detail.creatApplyName}}</div>
                <div class="detailTitle">加班申请人</div>
                <div class="detailContent">{{detail.extraWorkUser}}</div>
            </div>

            <div class="detailItem">
                <div class="detailTitle">申请处理方法</div>
                <div class="detailContent">{{handingMethod}}</div>
                <div class="detailTitle" v-show="detail.handingMethod=='4'">抵消迟到记录</div>
                <div class="detailContent" v-show="detail.handingMethod=='4'">{{detail.attrance}}</div>
            </div>

            <div class="detailItem" v-show="detail.projectID!=''">
                <div class="detailTitle">项目编号</div>
                <div class="detailContent">{{detail.projectID}}</div>
                <div class="detailTitle">项目名称</div>
                <div class="detailContent">{{detail.ProjectName}}</div>
            </div>

            <div class="detailItem">
                <div class="detailTitle">加班时间始</div>
                <div class="detailContent">{{detail.overtimeStart}}</div>
                <div class="detailTitle">加班时间止</div>
                <div class="detailContent">{{detail.overtimeEnd}}</div>
            </div>
...
export default {
    ...
    data(){
        return {
            ...
            detail:{
                ...
                creatApplyName:"",  //代申请人
                extraWorkUser:"",   //加班申请人
                handingMethod:"",   //申请处理方法  
                attrance:"",        //迟到记录
                projectID:"",       //项目编号
                ProjectName:"",     //项目名称
                overtimeStart:"",   //加班时间
                overtimeEnd:"",     //当日出勤信息
                extrWorkReason:"",  //加班事由概述
                ApplyName:"",
                CreateDate:"",
                Content:[],
                Images:[],
    ...
    methods:{
        confirm(){
            ...
            let id = this.$route.params.extraworkID;
        ...
        loadDetail(){
            let cosNum = this.userInfo.cosNum;
            let examineID = this.$route.params.extraworkID;
            let loading = showLoading();
            getExamineByID(cosNum,examineID).then((result)=>{
                ...
                for (let j = 0; j < Content.length; j++) {
                    if (Content[j].name=='代申请人') {
                        obj.creatApplyName = Content[j].value
                    }
                    if (Content[j].name=='加班申请人') {
                        obj.extraWorkUser = Content[j].value
                    }
                    if (Content[j].name=='申请处理方法') {
                        obj.handingMethod = Content[j].value
                    }
                    if (Content[j].name=='抵消迟到记录') {
                        obj.attrance = Content[j].value
                    }
                    if (Content[j].name=='项目编号') {
                        obj.projectID = Content[j].value
                    }
                    if (Content[j].name=='项目名称') {
                        obj.ProjectName = Content[j].value
                    }
                    if (Content[j].name=='加班时间始') {
                        obj.overtimeStart = Content[j].value
                    }
                    if (Content[j].name=='加班时间止') {
                        obj.overtimeEnd = Content[j].value
                    }
                    if (Content[j].name=='加班事由概述') {
                        obj.extrWorkReason = Content[j].value
                    }
                }
                obj.Images = [];
                if(obj.Img1){
