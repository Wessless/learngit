<template>
    <div class="businessApproveDetail">
        ...
        <div class="detailList">
            ...
            <div class="detailItem" v-for="item in detail.Content" :key="item.name">
                <div class="detailTitle">{{item.name}}</div>
                <div class="detailContent">{{item.value}}&nbsp;{{item.name=='出差天数'?'天':''}}</div>
            </div>
...
export default {
    ...
    methods:{
        confirm(){
            ...
            let id = this.$route.params.businessID;
        ...
        loadDetail(){
            let cosNum = this.userInfo.cosNum;
            let examineID = this.$route.params.businessID;
            let loading = showLoading();
