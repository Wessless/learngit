<template>
    <div class="MonthlyScoreDetail">
        ...
        <div class="midTableHeight summaryColor" style="width:100%;display:block;padding:0px 10px 20px 10px;">
            <el-table :data="monthlyScoreDetail" :cell-class-name="tableCellClassName" :header-cell-style="getRowStyle"
...
import { getMonthEvaluationDetail,addMonthEvaluation } from '@/js/api'
...
export default {
    ...
    methods:{
        confirm(){
            // staffID,managerID,points,strComment
            let staffID = this.item.StaffID;
            let managerID = this.userInfo.userStaffID;
            let strComment = this.strComment;
            let points = [];
            for (let i = 0; i < this.monthlyScoreDetail.length; i++) {
                points.push(Number(this.monthlyScoreDetail[i].Point));
            }
            let pointsStr = points.toString();

            let loading = showLoading();
            addMonthEvaluation(staffID,managerID,pointsStr,strComment).then((result)=>{
                ...
                console.log(err);
                closeLoading(loading);
                alertError(this,"2109");
            });

        },
        loadList(){
            this.monthlyScoreDetail = [];
            ...
            let isEvaluationOver;
            
            if (this.item.Status == "尚未考核") {
                isEvaluationOver = "0";
            }else{
                isEvaluationOver = "1";
            }
            let monthDate = this.item.MonthDate?this.item.MonthDate:"";
            let loading = showLoading();
            getMonthEvaluationDetail(staffID,managerID,isEvaluationOver,monthDate).then((result)=>{
                closeLoading(loading);
                this.monthlyScoreDetail = result.data.Details;
                this.strComment = result.data.Comment;
                console.log(result.data)
            }).catch((err)=>{
                console.log(err);
                closeLoading(loading);
                alertError(this,"1182");
            });
        },
