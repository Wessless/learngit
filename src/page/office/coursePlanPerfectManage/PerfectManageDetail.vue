    ...
    <div class="CoursePlanManageDetail">
        ...
        <el-form ref="form" :model="form" label-width="80px" style="width:100%;">
            ...
            <el-form-item label="分类">
                <el-select v-model="formInline.sortID" filterable size="medium" :disabled="$route.meta.type=='examined'" placeholder="请选择分类名称" style="width:650px;">
                    <el-option
                        v-for="item in formInline.sortArr"
                        :key="item.ID"
            ...
            <el-form-item label="课件列表" class="ReceiveApplyDetail_signatory">
                <el-table :data="form.link" border style="width:650px;margin-top:5px;margin-bottom:8px;line-height:20px;" max-height="400">
                    ...
                    <el-table-column prop="CoursewareFrom" label="来源" width="120" align="center">
                        <template slot-scope="scope">
                            <span>{{scope.row.CoursewareFrom=="Crisen"?"川迪课件库":"本地文件上传"}}</span>
                        </template>
                    </el-table-column>
                    <el-table-column label="操作" width="150" align="center">
                        <template slot-scope="scope">
                            ...
                            <el-button
                                size="mini"
                                type="danger"
                                v-show="$route.meta.type=='add'"
                                @click="delCourseware(scope.row,scope.$index)">删除</el-button>
                        </template>
                    </el-table-column>
                    <div style="padding:10px;" slot="append" v-show="$route.meta.type=='add'">
            ...
            <el-form-item label="教学视频" style="margin-top:5px;">
                 <el-input
                    style="width:650px;"
                    :rows="4"
                    :disabled="$route.meta.type=='examined'"
                    placeholder="请输入视频地址"
                    v-model="form.TeachVideo">
                    <template slot="append" v-if="$route.meta.type=='examined'&&form.TeachVideo!=''">
                        <span style="cursor:pointer;" @click="goTeachVideo">点击播放</span>
                    </template>
                </el-input>
            </el-form-item>
            <el-dialog
...
import { getThemeDatabaseDetail,getCategoryList,addOrUpdateThemeDatabase,getCourseList,getCourseFromLibrary,addSelectCourse } from '@/js/api'
...
export default {
    ...
    mounted(){
        if(this.$route.meta.type=="update"){
            this.title = "修改教学主题";
            this.formInline.sortID = this.$route.params.ID.split("&")[1];
            this.getThemeDatabaseDetail();
        }else if (this.$route.meta.type=="add") {
            this.title = "添加教学主题";
            this.isDisabled = false;
            this.getCategoryList();
        }else if (this.$route.meta.type=="examined") {
            this.title = "查看教学主题";
    ...
    methods:{ 
        confirm(){
            ...
            let json = {
                ...
                TeachVideo:this.form.TeachVideo,
                ThemeDataType:this.formInline.sortID
            }
            for (let i = 0; i < this.form.link.length; i++) {
                ...
                obj.Type = this.form.link[i].CoursewareFrom;
                link.push(obj);
            }
            if (link.length>0) {     
            ...
            addOrUpdateThemeDatabase(json).then((result)=>{
        ...
        //加载页面
        getThemeDatabaseDetail(){
            let Id = this.$route.params.ID.split("&")[0];
            let staffId = this.userInfo.userStaffID;
            let loading = showLoading();
            this.getCategoryList();
            getThemeDatabaseDetail(staffId,Id,this.pageNo,this.pageSize).then((result)=>{
                closeLoading(loading);
                let obj = result.data;
                console.log(obj)
                ...
                this.form.TeachVideo = obj.TeachVideo;
        ...
        getCategoryList(){
            getCategoryList(this.pageNo,this.pageSize).then((result)=>{
                this.formInline.sortArr = result.data.data;
            })
        },
        ...
        addCourseware(){
            ...
            if (this.form.radio == "0") {
                ...
                this.getCourseList();
                this.formInline.coursewareName = [];
        ...
        confirmAddSelectCourse(){
            ...
            addSelectCourse(json).then((result)=>{
                ...
                if(result.data.data.length>0&&result.data.data.length<=2){
                    this.$message({
                        message:"添加成功",
                        type:"success"
                    });
                ...
                }else if(result.data.Result == "2"||result.data.data.length>2){
                    this.$message({
                        message:"您最多可以从川迪课件库中添加两个课件",
                        type:"error"
                    });
        ...
        goTeachVideo(){
            // if (this.form.TeachVideo=='') {
            //     this.$message({
            //         message:"教学视频为空",
            //         type:"error"
            //     });
            // }else{
                window.open(this.form.TeachVideo);
            // }
        }
    }
}
